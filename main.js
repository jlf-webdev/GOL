...
// saving too much causes an error in localStorage


// TO BE DONE

// boxWidth < 2 only authorized up to 1360px wider
...
//document.addEventListener('resize',defineCanvasSize);
...
let smallPanel = document.getElementById('smallPanel');
...
// utiliser le canvasRatio pour déterminer la hauteur
function createSizeList2(){
  var width = canvas.width;
  var height = canvas.height;
  var sizeCounter = 0;
  for (var i=minSize; i<=maxSize; i++){
    if (width%i == 0 && height%i == 0 /*&& (width/i)%2 == 0 && (height/i)%2 == 0*/){
      sizeList[sizeCounter] = i ;
      sizeCounter = sizeCounter + 1;
    }
  }
  boxWidth = sizeList[boxSizeId];
}
...
function createSizeList(){
  ...
  for(i=minSize; i<=100; i++){
    ...
    while (found != 1 && width >= canvas.width-20 && height >= canvas.height-20){
      if(width % i == 0){
        if(height % i == 0 && i != lastSize){
...
function Size(boxSize, canvas_width, canvas_height){
